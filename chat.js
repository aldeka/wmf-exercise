$(document).ready(function(){
    var previewCharLength = 160;
    $.each(chatData.topics, function(i, topic){
        topic.id = topic.responses[0].id;
        topic.numComments = topic.responses.length;
        topic.firstpost = topic.responses[0];
        topic.firstpost.summary = topic.firstpost.posttext;
        if (topic.firstpost.posttext.length > previewCharLength) {
            topic.firstpost.summary = topic.firstpost.posttext.slice(0,160) + '...';
        }
        $('#topic-template').tmpl(topic).appendTo('#topics');

        $.each(topic.responses, function(i, reply){
            var target;
            if (reply.parentid == topic.id || reply.parentid === 0) {
                target = '#topic-' + topic.id.toString() + ' .replies';
            } else {
                target = '#' + reply.parentid.toString() + ' > .children';
            }
            $('#reply-template').tmpl(reply).appendTo(target);
        });
    });
    ...
    $('body').on('click', '.topic', function(e){
        $(this).find('.expander').toggleClass('expanded');
        var that = this;
        var replyToggler = function(){
            $(that).find('.first-post-trunc').toggle();
            $(that).find('.replies').toggle();
        };
        // didn't get animation working, so pause is unneeded
        window.setTimeout(replyToggler, 0);
    });
